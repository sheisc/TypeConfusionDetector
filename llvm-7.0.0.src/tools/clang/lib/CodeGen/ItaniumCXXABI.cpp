...
#if 1
  if(EnableCastStub){
//    llvm::Value *StubArgs[] = {SrcValue,Value};
//    CGF.EmitUNSWStubFun("__au_edu_unsw_dynamic_cast_stub", StubArgs);

    // to model __dynamic_cast(...)
    llvm::Value * srcPtr = CGF.Builder.CreateBitCast(SrcValue,CGF.VoidPtrTy);

#if 0
    if(llvm::ConstantInt *ci = dyn_cast<llvm::ConstantInt>(OffsetHint)){
      int64_t offset = ci->getValue().getSExtValue();
      //
      llvm::Value *minusOffset = llvm::ConstantInt::get(CGF.Int64Ty,-offset);

      llvm::Value *isNotNull = CGF.Builder.CreateIsNotNull(Value);
      llvm::Value *nullVal = llvm::Constant::getNullValue(Value->getType());

      llvm::BasicBlock *gepBlock = CGF.createBasicBlock();
      //llvm::BasicBlock *setNullBlock = CGF.createBasicBlock();
      llvm::BasicBlock *saved = CGF.Builder.GetInsertBlock();
      llvm::BasicBlock *conBlock = CGF.createBasicBlock();

      // if
      CGF.Builder.CreateCondBr(isNotNull, gepBlock,conBlock);
      // gepBlock
      CGF.EmitBlock(gepBlock);
      llvm::Value *gep = CGF.Builder.CreateGEP(srcPtr,minusOffset);
      llvm::Value *bitcast = CGF.Builder.CreateBitCast(gep, Value->getType());
      CGF.EmitBranch(conBlock);

//      // setNullBlock
//      CGF.EmitBlock(setNullBlock);
//      llvm::Value *nullVal = llvm::Constant::getNullValue(Value->getType());
//      CGF.EmitBranch(conBlock);

      //conBlock
      CGF.EmitBlock(conBlock);
      llvm::PHINode *phi = CGF.Builder.CreatePHI(Value->getType(),2);
      phi->addIncoming(bitcast,gepBlock);
      phi->addIncoming(nullVal,saved);
      //
      Value = phi;
...
#endif
    //
    llvm::Value * dstPtr = CGF.Builder.CreateBitCast(Value,CGF.VoidPtrTy);
    llvm::Value *StubArgs[] = { srcPtr, dstPtr };
    llvm::CallInst * call = CGF.EmitUNSWStubFun("__au_edu_unsw_dynamic_cast_stub", StubArgs);
    CGF.AddMetadataForUNSWStubCallInst(call,SrcValue->getType(),Value->getType());
