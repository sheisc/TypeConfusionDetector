...
#endif
#if 1
    llvm::Value * DstVal = Builder.CreateBitCast(Src, DstTy);
    if(EnableCastStub){
      if(isa<CXXReinterpretCastExpr>(CE) && DestTy->getAs<PointerType>() != nullptr
          /*&& DestTy->getPointeeType()->getAs<RecordType>()*/) {
      //if(DestTy->getAs<PointerType>() && DestTy->getPointeeType()->getAs<RecordType>()) {

        llvm::Value * srcPtr = Builder.CreateBitCast(Src,CGF.VoidPtrTy);
        llvm::Value * dstPtr = Builder.CreateBitCast(DstVal,CGF.VoidPtrTy);
        llvm::Value *StubArgs[] = { srcPtr, dstPtr };
        llvm::CallInst * call = CGF.EmitUNSWStubFun("__au_edu_unsw_reinterpret_cast_stub", StubArgs);

        CGF.AddMetadataForUNSWStubCallInst(call,Src->getType(),DstVal->getType());
      }      
    }
    return DstVal;
#endif    
...
#if 1
    if(EnableCastStub){
//      llvm::Value *StubArgs[] = { Base.getPointer(), Derived.getPointer() };
//      CGF.EmitUNSWStubFun("__au_edu_unsw_static_cast_stub", StubArgs);

      llvm::Value * srcPtr = Builder.CreateBitCast(Base.getPointer(),CGF.VoidPtrTy);
      llvm::Value * dstPtr = Builder.CreateBitCast(Derived.getPointer(),CGF.VoidPtrTy);            
      llvm::Value *StubArgs[] = { srcPtr, dstPtr };
      llvm::CallInst * call = CGF.EmitUNSWStubFun("__au_edu_unsw_static_cast_stub", StubArgs);

      CGF.AddMetadataForUNSWStubCallInst(call,Base.getPointer()->getType(),Derived.getPointer()->getType());
    }
#endif
