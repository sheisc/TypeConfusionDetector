  ...
  if(EnableCastStub){
    ...
    if (ClassTy) {
      ...
      if (ClassDecl && ClassDecl->isCompleteDefinition() &&
        ...
        llvm::Value * voidPtr = Builder.CreateBitCast(allocation.getPointer(),CGM.VoidPtrTy);
        llvm::Value *StubArgs[] = {voidPtr};
        const char * stub_func_name;
        if(E->getNumPlacementArgs() >= 1)
          stub_func_name = "__au_edu_unsw_placement_new_stub";
        else
          stub_func_name = "__au_edu_unsw_new_stub";
        llvm::CallInst *call = EmitUNSWNewExpression(stub_func_name, StubArgs);
        llvm::PointerType *dstTy = elementTy->getPointerTo(allocation.getAddressSpace());
        AddMetadataForUNSWStubCallInst(call,dstTy,dstTy);
        allocation = Address(call,allocation.getAlignment());
      }
    }
  }
#endif
...
#if 0
  if(EnableCastStub && E->getNumPlacementArgs() >= 1){
    auto *ClassTy = allocType->getAs<RecordType>();
    if (ClassTy) {
      const CXXRecordDecl *ClassDecl =
        cast<CXXRecordDecl>(ClassTy->getDecl());
      ...
      //llvm::Value *nullVal = llvm::Constant::getNullValue(elementTy);
      llvm::Value * voidPtr = Builder.CreateBitCast(resultPtr,CGM.VoidPtrTy);
      llvm::Value *StubArgs[] = {voidPtr};
      //HexEmitStubFun("__au_edu_unsw_placement_new_stub", StubArgs);
      const char * stub_func_name;
      stub_func_name = "__au_edu_unsw_placement_new_stub";

      llvm::CallInst * stub_call = EmitUNSWNewExpression(stub_func_name, StubArgs);
      return Builder.CreateBitCast(stub_call, resultPtr->getType());
    }
  }
#endif
  return resultPtr;
