...
#if 1 //added 2018.10.12
#include <sys/time.h>
void CodeGenFunction::AddMetadataForUNSWStubCallInst(llvm::CallInst *call, llvm::Type *srcTy, llvm::Type *dstTy){
  struct timespec tv;
  clock_gettime(CLOCK_REALTIME,&tv);
  uint64_t usec = tv.tv_sec*1000000000UL + tv.tv_nsec;
  llvm::LLVMContext &C = getLLVMContext();
  llvm::Value * srcMeta = llvm::Constant::getNullValue(srcTy);
  llvm::Value * dstMeta = llvm::Constant::getNullValue(dstTy);
  llvm::Value *sid = llvm::ConstantInt::get(llvm::Type::getInt64Ty(C),usec,false);
  llvm::MDNode * castMetadata = llvm::MDNode::get(C,llvm::ValueAsMetadata::get(srcMeta));
  call->setMetadata("SrcTypeInfo",castMetadata);
  castMetadata = llvm::MDNode::get(C,llvm::ValueAsMetadata::get(dstMeta));
  call->setMetadata("DstTypeInfo",castMetadata);
  castMetadata = llvm::MDNode::get(C,llvm::ValueAsMetadata::get(sid));
  call->setMetadata("UNSWSessionID",castMetadata);
}

llvm::CallInst * CodeGenFunction::EmitUNSWStubFun(StringRef FunName, ArrayRef<llvm::Value *> DynamicArgs){
  SmallVector<llvm::Value *, 4> Args;
  SmallVector<llvm::Type *, 4> ArgTypes;
  Args.reserve(DynamicArgs.size());
  ArgTypes.reserve(DynamicArgs.size());
  for (size_t i = 0, n = DynamicArgs.size(); i != n; ++i) {
    Args.push_back(DynamicArgs[i]);
    ArgTypes.push_back(VoidPtrTy);
  }
  llvm::FunctionType *FnType =
      llvm::FunctionType::get(CGM.VoidPtrTy, ArgTypes, false);
  llvm::AttrBuilder B;
  B.addAttribute(llvm::Attribute::UWTable);
...
llvm::CallInst * CodeGenFunction::EmitUNSWNewExpression(StringRef FunName, ArrayRef<llvm::Value *> DynamicArgs){
  return EmitUNSWStubFun(FunName, DynamicArgs);
}
#endif
#if 0
void CodeGenFunction::EmitUNSWStubFun(StringRef FunName, ArrayRef<llvm::Value *> DynamicArgs){
  SmallVector<llvm::Value *, 4> Args;
  SmallVector<llvm::Type *, 4> ArgTypes;
  Args.reserve(DynamicArgs.size());
  ArgTypes.reserve(DynamicArgs.size());
...
LValue CodeGenFunction::EmitCastLValue(const CastExpr *E) {
  ...
  case CK_BaseToDerived: {
    ...
    if(EnableCastStub){
        // static_cast<A&>()
//      llvm::Value *StubArgs[] = { LV.getAddress().getPointer(), Derived.getPointer() };
//      EmitUNSWStubFun("__au_edu_unsw_static_cast_stub", StubArgs);

      llvm::Value * srcPtr = Builder.CreateBitCast(LV.getAddress().getPointer(),VoidPtrTy);
      llvm::Value * dstPtr = Builder.CreateBitCast(Derived.getPointer(),VoidPtrTy);      
      llvm::Value *StubArgs[] = { srcPtr, dstPtr };
      llvm::CallInst * call = EmitUNSWStubFun("__au_edu_unsw_static_cast_stub", StubArgs);

      AddMetadataForUNSWStubCallInst(call,LV.getAddress().getPointer()->getType(),Derived.getPointer()->getType());
    }
#endif
