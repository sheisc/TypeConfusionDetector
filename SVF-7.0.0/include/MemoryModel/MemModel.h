...
#if 0
    /// Offsets of all fields of a struct
    std::vector<u32_t> orgIdx2FlattenFldInxVec;
#endif
    /// Offsets of all fields of a struct
...
#if 0 // added
    inline std::vector<u32_t>& getOrgIdx2FlattenFldInxVec() {
        return orgIdx2FlattenFldInxVec;
    }
#endif
    //@}
...
public:
...
#if 1
    const std::string getSymTypeName(SymID id){
        static const char * symTypeNames[] = {
          "BlackHole",
          "ConstantObj",
          "BlkPtr",
          "NullPtr",
        ...
        if(symTyMap.find(id) == symTyMap.end()){
            return "unknown";
        }else{
            return symTypeNames[symTyMap.find(id)->second];
        }
    }
    // added 2018.11.08
    llvm::StructType * getSubStructByBytes(llvm::StructType * baseTy, int64_t offset);
    Size_t getConstantOffsetOfGep(const llvm::Value * val);
#endif
    ...
    const inline std::vector<u32_t>& getOrgIdx2FlattenFldInxVec(const llvm::Type *T) {
        return getStructInfoIter(T)->second->getOrgIdx2FlattenFldInxVec();
    }
#endif
    //@}
