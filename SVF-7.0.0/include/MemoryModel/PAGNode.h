...
public:
    ...
    /// GepObjNode: tempory gep obj node for field sensitivity
...
public:
    ...
    static const std::string getPAGNodeKindName(int kind){
...
public:
    ...
    const std::string getValueName() {
        if (value && value->hasName())
            return value->getName().str() + "_" + llvm::itostr(ls.getFldIdx());
        return "offset_" + llvm::itostr(ls.getFldIdx());
    }
};
