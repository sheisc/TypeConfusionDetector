...
void SymbolTableInfo::collectArrayInfo(const ArrayType* ty) {
    ...
    for (u32_t j = 0; j < nfE; j++) {
        ...
        FieldInfo field(off, fieldTy, pair,elemStInfo->getFlattenFieldInfoVec()[j].getUnFlattenType());
#endif
...
void SymbolTableInfo::collectArrayInfo(const ArrayType* ty) {
    StInfo* stinfo = new StInfo();
    typeToFieldInfo[ty] = stinfo;
#if 0  // added 2018.11.14
    /// Array itself only has one field which is the inner most element
    stinfo->getOrgIdx2FlattenFldInxVec().push_back(0);
#endif
...
#endif
#if 1
        const Type *biggestSubTy;
        if(j == 0){
            biggestSubTy = elemTy;
        }else{
            biggestSubTy = fieldTy;
        }
        FieldInfo field(idx, off, fieldTy, pair, biggestSubTy,elemTy);
#endif
        stinfo->getFlattenFieldInfoVec().push_back(field);
    }
}
...
void SymbolTableInfo::collectStructInfo(const StructType *sty) {
    ...
    for (StructType::element_iterator it = sty->element_begin(), ie =
        ...
        if (isa<StructType>(et) || isa<ArrayType>(et)) {
#if 1   // added 2018.11.08   ERROR ; It should be removed.
            if(const ArrayType * at = dyn_cast<const ArrayType>(et)){
                et = at->getArrayElementType();
            }
#endif
...
#endif
#if 1   // added 2018.11.08
                const Type *unFlatten;
                if(it == sty->element_begin()){
                    unFlatten = sty;
                }else if(j == 0){
                    unFlatten = et;
                }else{
                    unFlatten = subStinfo->getFlattenFieldInfoVec()[j].getUnFlattenType();
                }
                FieldInfo field(off,elemTy,pair,unFlatten);
#endif
                stinfo->getFlattenFieldInfoVec().push_back(field);
            }
            nf += nfE;
            ...
            FieldInfo field(nf,et,pair,unFlatten);
#endif
            stinfo->getFlattenFieldInfoVec().push_back(field);
            ++nf;
        }
    ...
    for (StructType::element_iterator it = sty->element_begin(), ie =
        ...
        if (isa<StructType>(et) || isa<ArrayType>(et)) {
            ...
            for (u32_t j = 0; j < nfE; j++) {
...
#endif
#if 1 // modified 2018.11.19
                const Type *biggestSubTy;
                if(it == sty->element_begin() && j == 0){
                    biggestSubTy = sty;
                }else if(j == 0){
                    biggestSubTy = et;
                }else{
                ...
                FieldInfo field(fldIdx, off,elemTy,pair,biggestSubTy,sty);
#endif
                stinfo->getFlattenFieldInfoVec().push_back(field);
            }
            nf += nfE;
            ...
            FieldInfo field(nf, eOffsetInBytes,et,pair,biggestSubTy,sty);
#endif
            stinfo->getFlattenFieldInfoVec().push_back(field);
            ++nf;
        }
...
#endif
#if 1
    FieldInfo field(0, 0, ty, pair,ty,ty);
#endif
    stinfo->getFlattenFieldInfoVec().push_back(field);
}
#endif
...
bool SymbolTableInfo::computeGepOffset(const llvm::User *V, LocationSet& ls) {
    ...
    if(gepOp && dataLayout && gepOp->accumulateConstantOffset(*dataLayout,byteOffset)){
        bo = byteOffset.getSExtValue();
        // byte offset will be rounded up in TCD.cpp (GetAbstractObjectType())
        assert(ls.getByteOffset() == 0);
        ls.setByteOffset(bo);
        if(bo < 0){
          ls.setMinusOffset(bo);
        }
        StructType * structTy = analysisUtil::getBaseStructType((Value *)gepOp->getPointerOperand());
        if(structTy && bo >= 0){            
            ls.setBaseType(structTy);
            Size_t sz = SymbolTableInfo::Symbolnfo()->getTypeSizeInBytes(structTy);
            bo %= sz;
            Size_t idx = analysisUtil::getFldIdxViaByteOffset(structTy,bo);
            if(idx >= 0){
                ls.setFldIdx(idx);                
            }
            return true;
    ...
    for (bridge_gep_iterator gi = bridge_gep_begin(*V), ge = bridge_gep_end(*V);
        ...
        if (const StructType *ST = dyn_cast<StructType>(*gi) ) {
            ...
            ls.setFldIdx(ls.getFldIdx() + so[idx]);
        }
    }
    return true;
}
...
#endif
#if 1 // added 2018.11.08
    fields.push_back(LocationSet(0,0,nullptr,0));//V->getType()));
#endif
...
#endif
#if 1 //modified
    LocationSet ret_ls(offset,ls.getByteOffset(),ls.getBaseType(),ls.getMinusOffset());
    return ret_ls;
#endif
}
...
bool SymbolTableInfo::isConstantObjSym(const Value *val) {
    if (const GlobalVariable* v = dyn_cast<GlobalVariable>(val)) {
        if (cppUtil::isValVtbl(const_cast<GlobalVariable*>(v)))
            return false;
        else if (!v->hasInitializer()) // extern int number;
            return true;
        else {
...
void SymbolTableInfo::handleCE(const Value *val) {
    if (const Constant* ref = dyn_cast<Constant>(val)) {
        if (const ConstantExpr* ce = isGepConstantExpr(ref)) {
            // getelementptr %struct.Data, %struct.Data* @d, i32 0, i32 1
            DBOUT(DMemModelCE,
            ...
            // bitcast (%struct.Info* @info to i8*)
            DBOUT(DMemModelCE,
                  outs() << "handle constant expression " << *ref << "\n");
            collectVal(ce);
            collectVal(ce->getOperand(0));
...
void SymbolTableInfo::handleGlobalCE(const GlobalVariable *G) {
    ...
    // G->getType() must be a PointerType.
    assert(G->getType()->getElementType() == G->getType()->getContainedType(0));
...
void SymbolTableInfo::printFlattenFields(const llvm::Type* type) {
     ...
     */
        ...
        for(std::vector<FieldInfo>::iterator it = finfo.begin(), eit = finfo.end();
            ...
            outs() << "\n\t\tUnflatten Type: ";
#if 1
            (*it).getUnFlattenType()->print(outs());
            outs() << "\n\n";
#endif
        }
        outs() << "\n";
...
void SymbolTableInfo::printFlattenFields(const llvm::Type* type) {
    ...
    if(const ArrayType *at = dyn_cast<ArrayType> (type)) {
        ...
        for(std::vector<FieldInfo>::iterator it = finfo.begin(), eit = finfo.end();
                it!=eit; ++it, field_idx++) {
            llvm::Type *innerTy = (Type *) (*it).getFlattenElemTy();
            outs() << " \tField_idx = " << (*it).getFlattenFldIdx() << " [offset: " << (*it).getFlattenByteOffset();
            outs() << ", field type: ";
            innerTy->print(outs());
            outs() << ", field size: " << getTypeSizeInBytes(innerTy);
            outs() << ", field stride pair: ";
            for(FieldInfo::ElemNumStridePairVec::const_iterator pit = (*it).elemStridePairBegin(),
                    peit = (*it).elemStridePairEnd(); pit!=peit; ++pit) {
                outs() << "[ " << pit->first << ", " << pit->second << " ] ";
...
Size_t SymbolTableInfo::getConstantOffsetOfGep(const Value * val){
    const GEPOperator *gepOp = dyn_cast<const GEPOperator>(val);
    if(gepOp){
        APInt byteOffset(64,0,true);
        DataLayout * dataLayout = getDataLayout(getModule().getMainLLVMModule());
        if(gepOp && dataLayout && gepOp->accumulateConstantOffset(*dataLayout,byteOffset)){
            // byte offset will be rounded up in TCD.cpp (GetAbstractObjectType())
            return byteOffset.getSExtValue();
        }
    }
    return 0;
}
