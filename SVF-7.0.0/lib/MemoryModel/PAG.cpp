...
PAG* PAG::pag = NULL;
#if 1   // 2018.11.08
static const char * valueTyName[] = {
    #define HANDLE_VALUE(Name) #Name,
    #include <llvm/IR/Value.def>
};
static const char * instName[] = {
    #define HANDLE_TERM_INST(N, OPC, CLASS) #CLASS,
    #define HANDLE_BINARY_INST(N, OPC, CLASS) #CLASS,
    #define HANDLE_MEMORY_INST(N, OPC, CLASS) #CLASS,
    #define HANDLE_CAST_INST(N, OPC, CLASS) #CLASS,
    #define HANDLE_FUNCLETPAD_INST(N, OPC, CLASS) #CLASS,
    #define HANDLE_OTHER_INST(N, OPC, CLASS) #CLASS,
    #include <llvm/IR/Instruction.def>
};

#define  LEN(arr)   (sizeof(arr)/sizeof(arr[0]))

static const char * getValueTyName(const Value * val){
    if(val){
        unsigned int vid = val->getValueID();
        //DBOUT(DMemModel,outs() <<"vid is " << vid << ":"<<LEN(valueTyName) << ":"<<LEN(instName)<<"\n");
        if(vid < LEN(valueTyName)){
            return valueTyName[vid];
        }else{
            return instName[ (vid - LEN(valueTyName)) % LEN(instName) ];
        }
    }else{
        return "";
    }
...
#endif
#if 1 // modified 2018.11.04
    if(hasIntraEdge(baseNode, dstNode, PAGEdge::NormalGep))
        return false;
    else
        return addEdge(baseNode, dstNode, new NormalGepPE(baseNode, dstNode, baseLS+ls));
#endif
}
...
#endif
#if 1  // modified 2018.11.07
    if(geps.empty()){
//        Value * val = (Value*)node->getValue();
//        Type * baseTy = val->getType()->getContainedType(0);
//        //
//        BitCastOperator * bitcast = dyn_cast<BitCastOperator>(val);
//        if(bitcast && baseTy->isIntegerTy()){
//            baseTy = bitcast->getSrcTy()->getContainedType(0);
//        }
//        while(ArrayType * at = dyn_cast<ArrayType>(baseTy)){
//            baseTy = at->getArrayElementType();
//        }
//        return LocationSet(0,0,baseTy);
        Type * baseTy = nullptr;
        if(node->hasValue()){
            baseTy = analysisUtil::getBaseStructType((Value *) node->getValue());
        }
        return LocationSet(0,0,baseTy,0);
    }
#endif
...
#if 1
void PAG::printNode(NodeID id){
    SymbolTableInfo* st = SymbolTableInfo::Symbolnfo();
    PAGNode * node = this->getPAGNode(id);
    DBOUT(DGENERAL, outs() << "Node Id: \t" << id  << " \n");
    DBOUT(DGENERAL,outs() << "\t PNODEK:" << PAGNode::getPAGNodeKindName(node->getNodeKind()) << "\n");
    DBOUT(DGENERAL,outs() << "\t SYMTYPE:" << st->getSymTypeName(id) << "\n");
    if(const ObjPN * obj = dyn_cast<ObjPN>(node)){
        DBOUT(DGENERAL, outs() << "\t is ObjPN \n");
    }else{
        DBOUT(DGENERAL, outs() << "\t is not ObjPN \n");
    }
    if(const ValPN * val = dyn_cast<ValPN>(node)){
        DBOUT(DGENERAL, outs() << "\t is ValPN \n");
    }else{
        DBOUT(DGENERAL, outs() << "\t is not ValPN \n");
    }
    if(pag->isValidTopLevelPtr(node)){
        DBOUT(DGENERAL, outs() << "\t is ValidTopLevelPtr \n");
    }else{
        DBOUT(DGENERAL, outs() << "\t is not ValidTopLevelPtr \n");
    }
    if(node->isPointer()){
        DBOUT(DGENERAL, outs() << "\t is Pointer \n");
    }else{
        DBOUT(DGENERAL, outs() << "\t is not Pointer \n");
    }
    if(node->isAddressTakenPtr()){
        DBOUT(DGENERAL, outs() << "\t is AddressTakenPtr \n");
    }else{
        DBOUT(DGENERAL, outs() << "\t is not AddressTakenPtr \n");
    }

    if(node->isTopLevelPtr()){
        DBOUT(DGENERAL, outs() << "\t is TopLevelPtr \n");
    }else{
        DBOUT(DGENERAL, outs() << "\t is not TopLevelPtr \n");
    }

    if(node->hasValue()){
        DBOUT(DGENERAL, outs() << "\t" << *(node->getValue()) << "\n");
        DBOUT(DGENERAL, outs() << "\t" << analysisUtil::getSourceLoc(node->getValue()) << "\n");
    }else{
        DBOUT(DGENERAL, outs() << "\t does not have Value\n");
        return;
    }
    if(pag->hasValueNode(node->getValue())){
        DBOUT(DGENERAL, outs() << "\t(value)Node id: " <<  pag->getValueNode(node->getValue()) << "\n");
    }else{
        DBOUT(DGENERAL, outs() << "\t does not have Value Node\n");
    }
    if(st->objSyms().find(node->getValue()) != st->objSyms().end()){
        DBOUT(DGENERAL, outs() << "\t(object)Node id: " <<  pag->getObjectNode(node->getValue()) << "\n");
    }else{
        DBOUT(DGENERAL, outs() << "\t does not have Object Node\n");
    }
    if(const GepObjPN * gepObj = dyn_cast<const GepObjPN>(node)){
        Type * baseTy = analysisUtil::getAbstractObjectType((Value *)node->getValue());
        if(baseTy){
            Size_t byteOffset = gepObj->getLocationSet().getByteOffset();
            Size_t sz = SymbolTableInfo::Symbolnfo()->getTypeSizeInBytes(baseTy);
            DBOUT(DGENERAL, outs() << "\t Base Type from PTA: " << *baseTy << "\n");
            DBOUT(DGENERAL, outs() << "\t Type Size: " << sz << "\n");
            DBOUT(DGENERAL, outs() << "\t Field Index from LocationSet: " << gepObj->getLocationSet().getFldIdx() << "\n");
            Type * baseTyFromLS = gepObj->getLocationSet().getBaseType();
            if(baseTyFromLS){
                DBOUT(DGENERAL, outs() << "\t Base Type from LocationSet: " << *baseTyFromLS << "\n");
            }
            DBOUT(DGENERAL, outs() << "\t Byte Offset from LocationSet: " << byteOffset << "\n");
            DBOUT(DGENERAL, outs() << "\t Byte Offset % Type Size = : " << (byteOffset % sz) << "\n");
        }
    }
...
void PAG::print() {
  ...
  for (PAGEdge::PAGEdgeSetTy::iterator iter = addrs.begin(), eiter =
    ...
    outs() << (*iter)->getSrcID() << " -- Addr --> " << (*iter)->getDstID()
        << "\n";
#if 1
    printNode((*iter)->getSrcID());
    printNode((*iter)->getDstID());
#endif
  }
...
#if 1  // added by iron, 2018.05.13

std::string doGetNodeLabel(PAGNode *node) {
    std::string str;
    raw_string_ostream rawstr(str);
    rawstr << PAGNode::getPAGNodeKindName(node->getNodeKind()) << "\n";
    rawstr << "NodeID:" << node->getId() << "\n";

    if(!isa<DummyValPN>(node) && !isa<DummyObjPN>(node)){
        rawstr << node->getValueName() <<":";
        if(!SymbolTableInfo::isBlkObjOrConstantObj(node->getId())){
            rawstr << analysisUtil::getSourceLoc(node->getValue()) << "\n";
        }
        if(node->hasValue()){
            rawstr << getValueTyName(node->getValue()) << "\n";
        }
    }else{
        rawstr << "";
    }
