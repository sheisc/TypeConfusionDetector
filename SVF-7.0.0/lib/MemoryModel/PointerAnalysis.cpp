...
void BVDataPTAImpl::writeToFile(const string& filename) {
    ...
    for (NodeID i = firstGepObjNode, e = pag->getTotalNodeNum(); i != e; ++i) {
        ...
        if (gepObjPN) {
            ...
            F.os() << gepObjPN->getLocationSet().getFldIdx() << "\n";
        }
    }
...
bool BVDataPTAImpl::readFromFile(const string& filename) {
    ...
    while (F.good()) {
        ...
        NodeID n = pag->getGepObjNode(pag->getObject(base), LocationSet(offset,0,nullptr,0));
#endif
...
#endif
#if 1 // sorted version
    std::map<NodeID, PAGNode*> pairs(pag->begin(),pag->end());
    for(auto &p: pairs) {
      DBOUT(DGENERAL,outs() << "\n----------------------------- Points-to for PAG Node "
            << p.first << " -----------------------------------------\n");
      dumpPts(p.first, this->getPts(p.first));

      DBOUT(DGENERAL,outs() << "Pointer:\n");
      this->getPAG()->printNode(p.first);

      DBOUT(DGENERAL,outs() << this->getPts(p.first).count() << " Pointee(s):\n");
      for(auto n: this->getPts(p.first)){
          this->getPAG()->printNode(n);
      }
      //outs() << "----------------------------------------------\n";
    }
#endif
