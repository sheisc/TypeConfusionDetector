 ...
 */
NodeID PAGBuilder::getGlobalVarField(const GlobalVariable *gvar, u32_t offset, Size_t byteOffset) {
    ...
    /// then we need to create a gep node for this field
        ...
        return pag->getGepValNode(gvar, LocationSet(offset,byteOffset,(Type *)gvartype,0), gvartype, offset);
#endif
    }
}
...
void PAGBuilder::InitialGlobal(const GlobalVariable *gvar, Constant *C,
                               u32_t offset, Size_t byteOffset) {
    DBOUT(DPAGBuild,
    ...
    if (C->getType()->isSingleValueType() && isa<PointerType>(C->getType())) {
        ...
        NodeID field = getGlobalVarField(gvar, offset, byteOffset);
    ...
    } else if (ConstantArray * conArray = dyn_cast<ConstantArray>(C)) {
        ArrayType * at = conArray->getType();
        if (cppUtil::isValVtbl(gvar) == false){
            Size_t eleSz = SymbolTableInfo::Symbolnfo()->getTypeSizeInBytes(at->getArrayElementType());
            for (u32_t i = 0, e = C->getNumOperands(); i != e; i++){
                InitialGlobal(gvar, cast<Constant>(C->getOperand(i)), offset, byteOffset + i*eleSz);
            }
        }else{
            byteOffset += SymbolTableInfo::Symbolnfo()->getTypeSizeInBytes(at);
        }
    } else if (isa<ConstantStruct>(C)) {
//        const StructType *sty = cast<StructType>(C->getType());
//        const std::vector<u32_t>& offsetvect =
//            SymbolTableInfo::Symbolnfo()->getStructOffsetVec(sty);
//        Size_t preByteOffset = 0;
//        for (u32_t i = 0, e = C->getNumOperands(); i != e; i++) {
//            u32_t off = offsetvect[i];
//            Type * eleTy = C->getOperand(i)->getType();
//            Size_t eleSz = SymbolTableInfo::Symbolnfo()->getTypeSizeInBytes(eleTy);
//            InitialGlobal(gvar, cast<Constant>(C->getOperand(i)), offset + off, byteOffset + preByteOffset);
//            preByteOffset += eleSz;
//        }
          const StructType *sty = cast<StructType>(C->getType());
          SymbolTableInfo * symTable = SymbolTableInfo::Symbolnfo();
          const DataLayout * dataLayout = symTable->getDataLayout(symTable->getModule().getMainLLVMModule());
          const StructLayout *structLayout = dataLayout->getStructLayout((StructType *)sty);
#if 0
          const std::vector<u32_t>& offsetvect =
              SymbolTableInfo::Symbolnfo()->getStructOffsetVec(sty);
#endif
#if 1  // modifed 2018.11.14
          const std::vector<u32_t>& offsetvect =
          ...
          for (u32_t i = 0, e = C->getNumOperands(); i != e; i++) {
              ...
              InitialGlobal(gvar, cast<Constant>(C->getOperand(i)), offset + off, byteOffset + eleOffset);
          }
    ...
    if (isHeapAllocOrStaticExtCall(cs)) {
...
#if 0 // added 2018.11.12
            if(callee && isNewExpressionStub(callee->getName())){
        ...
        if(isExtCall(callee)) {
            ...
            case ExtAPI::EFT_STD_RB_TREE_INSERT_AND_REBALANCE: {
                ...
                Size_t offset = pag->getLocationSetFromBaseNode(vnArg3).getFldIdx();
            ...
            case ExtAPI::EFT_STD_RB_TREE_INCREMENT: {
                ...
                Size_t offset = pag->getLocationSetFromBaseNode(vnArg).getFldIdx();
...
#if 0   // we have modified the clang to model __dynamic_cast more precisely
...
#endif
#if 1   // added 2018.11.08   // FIXME
        pag->addNormalGepEdge(srcID, dstID, LocationSet(offsetOrCSId,0,nullptr,0));
#endif
