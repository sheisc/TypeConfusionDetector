...
#if 1 // added 2018.11.07
static bool isOperatorNewInClass(llvm::StringRef name){
    return name.startswith(CUSTOMED_OPERATOR_NEW_ZN) &&
                      (name.contains(CUSTOMED_OPERATOR_NEW_nwEm) || name.contains(CUSTOMED_OPERATOR_NEW_naEm));
}
/*
    Although the names of global operator new  are already in info[],
    if they are overloaded by a C++ application (that is, with function definition),
    they will be treated as EFT_OTHER in get_type().
    To avoid such a result, isOperatorNewInGlobalScope() should be called in isCustomizedCXXOperatorNew().
 */
static bool isOperatorNewInGlobalScope(llvm::StringRef name){
    return name.startswith(CUSTOMED_OPERATOR_NEW_Znwm) || name.startswith(CUSTOMED_OPERATOR_NEW_Znam)
                  || name.startswith(CUSTOMED_OPERATOR_NEW_Znaj) || name.startswith(CUSTOMED_OPERATOR_NEW_Znwj);
}
bool isNewExpressionStub(llvm::StringRef name){
    return name.equals(PLACEMENT_NEW_STUB) || name.equals(NEW_STUB);
}

bool ExtAPI::isCustomizedCXXOperatorNew(const llvm::Function *F)const{
    llvm::StringRef name = F->getName();
    // FIXME: Stronger String Comparison is needed here.
    bool ret =  (isOperatorNewInClass(name) ||  isOperatorNewInGlobalScope(name) || isNewExpressionStub(name))
                &&
...
ExtAPI::extf_t ExtAPI::get_type(const llvm::Function *F) const {
    assert(F);
#if 1// added 2018.11.06
    if(isCustomizedCXXOperatorNew(F)){
        //FIXME: Stronger String Comparison is needed here.
        //llvm::StringMap<extf_t> * map = (llvm::StringMap<extf_t> *) &info;
        //(*map)[F->getName().data()] = EFT_ALLOC;
        return EFT_ALLOC;
    }
#endif
