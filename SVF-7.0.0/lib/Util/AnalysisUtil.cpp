...
Instruction * analysisUtil::findNextInstruction(Instruction *CurInst) {
...
llvm::Type * analysisUtil::getAbstractObjectType(llvm::Value * val){
    llvm::Type *pointeeType = nullptr;
    if(CallInst * call = dyn_cast<CallInst>(val)){
        /*
            %2 = call i8* @_Znwm(i64 16) #11
            %3 = bitcast i8* %2 to %struct.B*
           */
        Function * func = call->getCalledFunction();
        if(analysisUtil::isHeapAllocExtCallViaRet(call)){
            if(func && isNewExpressionStub(func->getName())){
                if(PointerType * t = getPointerTypeViaMetadata(call,DST_TYPE_INFO_METADATA)){
                    pointeeType = t->getPointerElementType();
                }
            }
//            else if(BitCastInst * bitcast = dyn_cast<BitCastInst>(findNextInstruction(call))){
//                if(PointerType * t = dyn_cast<PointerType>(bitcast->getDestTy())){
//                    pointeeType = t->getPointerElementType();
//                }
//            }
        }
        ...
        // @c2 = dso_local global %struct.C zeroinitializer, align 8
...
llvm::StructType * analysisUtil::getBaseStructType(llvm::Value * val){
    if(Type * ty = getAbstractObjectType(val)){
      while(ArrayType * at = dyn_cast_or_null<ArrayType>(ty)){
        ty = at->getArrayElementType();
      }
      return dyn_cast<StructType>(ty);
    }
    if(PointerType * pt = dyn_cast<PointerType>(val->getType())){
        Type * baseTy = pt->getPointerElementType();
        BitCastOperator * bitcast = dyn_cast<BitCastOperator>(val);
        // %8 = bitcast %class.C* %5 to i8*
...
/*
    ...
    @return
      ...
      >= 0    Field index
 */
Size_t analysisUtil::getFldIdxViaByteOffset(llvm::StructType * baseTy,Size_t byteoffset){
    StInfo * info = SymbolTableInfo::Symbolnfo()->getStructInfo(baseTy);
...
llvm::Type * analysisUtil::getInnermostType(llvm::Type * ty){
    while(ty->getNumContainedTypes() > 0 && ty->isAggregateType()){
        ty = ty->getContainedType(0);
    }
    return ty;
}
//bool isSVFCurrentDebugType(const char *Type){
//    /*
//       See /home/iron/github/PrivateHexType/llvm/lib/Support/Debug.cpp
//            bool isCurrentDebugType(const char *DebugType)
//                return true if string matches or the container is empty
//    */
//    #define D_PLACEHOLDER   "place holder"
