...
#if 1
/*
  In fact, this function has been defined in libclang_rt.cast_stub-x86_64.a.
  We redefine it to achieve the effect of both EFT_ALLOC and EFT_L_A0 (see Util/ExtAPI.cpp)

  extern "C" void * __au_edu_unsw_placement_new_stub( void *res){
    return (void*)res;
  }
 */
void LLVMModuleSet::addPlacemnetNewStub(){
    Module & M = *(modules[0].get());
    // char *
    Type * i8ptr = PointerType::getInt8PtrTy(M.getContext());
    Function *newStub = (Function*)M.getOrInsertFunction(
          PLACEMENT_NEW_STUB,
          i8ptr,
          i8ptr
          );
    newStub->setCallingConv(CallingConv::C);
    BasicBlock* block = BasicBlock::Create(M.getContext(), "entry", newStub);
    IRBuilder<> Builder(block);
    // return;
    Builder.CreateRet(newStub->arg_begin());
}
/*
   Add svf.main() to initialize all C++ objects and then call main()
 */
...
#endif
/*
    Collect all functions/global variables/global aliases from different modules.
 */
void LLVMModuleSet::initialize() {
    for (u32_t i = 0; i < moduleNum; ++i) {
        ...
        /*
          ...
          -------------------------------------------------------------------------
          @g = weak dso_local alias void (), void ()* @f12345678
         */
        /// GlobalAlias
...
/*
    (1) Handle functions which have both definitions and declarations.
    (2) Set up mappings between definitions and declarations
 */
void LLVMModuleSet::buildFunToFunMap() {
    ...
    // Find those functions that have both function definitions and declarations
    ...
    // ONLY map names in intersectNames to def/decls
    // one function name ---> one function definition
    // one function name ---> multiple function declarations
    ///// name to def map
...
/*
  Redefinitions are allowed here.
    1.cpp
          int number = 30;
    2.cpp
          int number = 50;
  Redefinitions might occur when there are several dynamic libraries.
 */
void LLVMModuleSet::buildGlobalDefToRepMap() {
    typedef std::map<string, std::set<GlobalVariable*>> NameToGlobalsMapTy;
    // collect global variables with the same name
    ...
    // The first global variable with initializer is chosen as the representitive
    for (NameToGlobalsMapTy::iterator it = nameToGlobalsMap.begin(),
            eit = nameToGlobalsMap.end(); it != eit; ++it) {
        // pick up the representitive
        ...
        // map other global variables to the representitive
...
#endif
#if 1
        WriteBitcodeToFile(*mod, OS, true);
#endif
        OS.flush();
    }
}
