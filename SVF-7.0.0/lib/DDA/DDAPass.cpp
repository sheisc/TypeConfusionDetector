...
#if 1
static CastStubInfoCollector * collector;
#endif
...
void DDAPass::runPointerAnalysis(SVFModule module, u32_t kind)
{
    ...
    if(WPANUM) {
...
#if 1
        if(!collector){
            collector = new CastStubInfoCollector(_pta);
            // Collect cast information of every module
            for (u32_t i = 0; i < module.getModuleNum(); ++i) {
                Module *m = module.getModule(i);
                collector->runOnModule(*m);
            }            
        }
#endif
        ///compute points-to        
        ...
        _pta->finalize();
#if 1
        // Type confusion detection should be done AFTER _pta->finalize()

        if(IsDetectTC()){
            collector->SetPTA(_pta);
            collector->PrintStatistics();
            collector->DetectTypeConfustion();
            //FIXME: delete collector.
        }
#endif
        if(printCPts)
            _pta->dumpCPts();
