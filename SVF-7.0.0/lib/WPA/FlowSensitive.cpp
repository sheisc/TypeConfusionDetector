...
#if 0
    for(PAG::const_iterator pit = pag->begin(), pe = pag->end(); pit != pe; ++pit){
        for(SVFG::const_iterator sit = svfg->begin(), se = svfg->end(); sit != se; ++sit){
            analysisUtil::dumpSet(this->getDFInPtsSet(sit->second,pit->first),llvm::outs());
        }
    }
    for(PAG::const_iterator pit = pag->begin(), pe = pag->end(); pit != pe; ++pit){
        ...
        if (ObjPN* fiNode = llvm::dyn_cast<ObjPN>(node)){
          bool ret = isNonPointerObj(pit->first);
          llvm::outs() << id << ":" << ret << "\n";
        }
    }
#endif
